...
- `apps/desktop-shell/src/main.ts` validates every payload-bearing IPC channel before touching windows, files, Git, terminals, or workspace backups.
- Invalid payloads are rejected and logged through the desktop logger for auditability.

## Preload Responsibilities
- `apps/desktop-shell/src/preload.ts` exposes a minimal API:
